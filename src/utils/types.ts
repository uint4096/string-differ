...
export interface CharOperations<T extends Operations> {
    type: T;
    value: T extends Extract<Operations, 'delete' | 'retain'>
        ? number
        : string;
}
...
interface RangeOperationsType<T extends Operations> { type : T };

type RangeOperationsBase<T extends Operations, K extends string | number = number> =
    T extends Extract<Operations, 'delete' | 'retain'>
        ? RangeOperationsType<T> & { startIndex: K }
        : RangeOperationsType<T> & { value: string }

export type RangeOperations<T extends Operations, K extends string | number = number> = 
    T extends Extract<Operations, 'delete' | 'retain'>
        ? K extends string
